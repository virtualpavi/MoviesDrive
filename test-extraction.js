...
/**
 * Test script to verify stream extraction for a specific IMDB ID
 * Usage: node test-extraction.js [IMDB_ID]
 */
...
if (args.length === 0) {
  console.error('Usage: node test-extraction.js [IMDB_ID]');
  console.error('');
  console.error('Example:');
  console.error('  node test-extraction.js tt32820897');
  process.exit(1);
}
...
async function testExtraction() {
  ...
  try {
    ...
    console.log('');
    console.log(`✓ Found ${streams.length} unique stream(s):`);
    console.log('');
    ...
    console.log('');
    console.log('📈 Summary:');
    const directStreams = streams.filter(s => !s.source.includes('Wrapper'));
    const wrapperStreams = streams.filter(s => s.source.includes('Wrapper'));
    
    console.log(`  - Direct streaming URLs: ${directStreams.length}`);
    console.log(`  - Wrapper URLs (fallback): ${wrapperStreams.length}`);
    
    if (directStreams.length > 0) {
      console.log('');
      console.log('✅ SUCCESS: Found direct streaming URLs!');
    } else {
      console.log('');
      console.log('⚠️ WARNING: Only wrapper URLs found. Extraction may need improvement.');
    }
